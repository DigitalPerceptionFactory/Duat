...
namespace Duat::Geometry {
	...
	struct Fibre
	{
		...
		Fibre() = default;
		Fibre(Utility::Hierarchy<Sekai>& cone, unsigned int node_index, float from = 0.0f, float to = 1.0f,
			Fibre::Type type = Fibre::Type::Children)
		{
			Init(cone, node_index, from, to, type);
		}

		void Append(Utility::Hierarchy<Sekai>& cone, unsigned int node_index, float from, float to, Fibre::Type type);
		void Init(Utility::Hierarchy<Sekai>& cone, unsigned int node_index, float from, float to, Fibre::Type type);
		Utility::ring<unsigned int> nodes;
	private:
		void generate_peers_rec(Utility::Hierarchy<Sekai>& cone, Utility::ring<int>& peers, int parent_count, int index);
	};
	...
	struct Isekai : public Utility::Hierarchy<Sekai> {
		Isekai();
		...
		std::pair<Utility::ring<Vertex>, Utility::ring<unsigned int>> GetMesh(
			Topology topology = Topology::TriangleList);
