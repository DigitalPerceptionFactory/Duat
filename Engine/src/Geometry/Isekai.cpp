...
namespace Duat::Geometry {
	...
	int Isekai::Create(std::string name,
	...
	{
		auto& sekai = Get(Hierarchy<Sekai>::Create(name));
	...
	int Isekai::CreateAndGo(std::string name, DirectX::XMFLOAT3 position, DirectX::XMFLOAT3 rotation, DirectX::XMFLOAT3 scale, DirectX::INT3 posRef, DirectX::INT3 rotRef, DirectX::INT3 scaRef)
	{
		Create(name, position, rotation, scale, posRef, rotRef, scaRef);
		return Go(name);
	}

	void Isekai::CreateCurve(std::function<DirectX::XMFLOAT3(float)> builder_function, Fibre x_axis, Fibre y_axis, Fibre z_axis, AxisMask x_mask, AxisMask y_mask, AxisMask z_mask)
	{
	...
	void Isekai::AddPatch(Patch p)
	{
		m_surface.push_back(p);
	}

	std::pair<ring<Geometry::Vertex>, ring<unsigned int>> Isekai::GetMesh(Topology topology)
	{
	...
	void Isekai::clamp_360(DirectX::INT3 index)
	{
	...
	DirectX::INT3 Isekai::GetPosRef(DirectX::INT3 index)
	{
		resolve_index(index);
	...
	DirectX::INT3 Isekai::GetRotRef(DirectX::INT3 index)
	{
		resolve_index(index);
	...
	DirectX::INT3 Isekai::GetScaRef(DirectX::INT3 index)
	{
		resolve_index(index);
	...
	DirectX::XMFLOAT3 Isekai::GetPosition(DirectX::INT3 index, Setup setup)
	{
		/*
	...
	DirectX::XMFLOAT3 Isekai::GetPosition(DirectX::XMFLOAT3 point, Setup setup)
	{
	...
	DirectX::XMFLOAT2 Isekai::GetPosition(DirectX::XMFLOAT2 point, Setup setup)
	{
		resolve_reference(0, setup);
		// global metrics
	...
	DirectX::XMFLOAT3 Isekai::GetRotation(DirectX::INT3 index, Setup setup)
	{
		resolve_all(index, setup);
	...
	DirectX::XMFLOAT3 Isekai::GetScale(DirectX::INT3 index, Setup setup)
	{
		resolve_all(index, setup);
	...
	Cube<float> Isekai::GetExceed(DirectX::INT3 index, Setup setup)
	{
		resolve_all(index, setup);
	...
	Cube<float> Isekai::GetExceed(DirectX::XMFLOAT3 point, Setup setup)
	{
	...
	Cube<float> Isekai::GetExceed(DirectX::XMFLOAT2 point, Setup setup)
	{
		// to prevent negative setup values
		resolve_reference(0, setup);
		XMFLOAT3 ref_boundary = get_global_boundary(setup.rotRef, setup.scaRef);
	...
	bool Isekai::Contains(DirectX::INT3 index, Setup setup)
	{
	...
	bool Isekai::Contains(DirectX::XMFLOAT3 point, Setup setup)
	{
	...
	bool Isekai::Contains(DirectX::XMFLOAT2 point, Setup setup)
	{
		Cube<float> exc = GetExceed(point, setup);
	...
	DirectX::XMFLOAT3 Isekai::GetOrigin(DirectX::INT3 index)
	{
		resolve_index(index);
		return {
	...
	DirectX::XMFLOAT3 Isekai::GetOriginPosition(DirectX::INT3 index, Setup setup)
	{
		resolve_all(index, setup);
	...
	DirectX::XMMATRIX Isekai::GetMatrix()
	{
		return GetMatrix(GetIndex());
	}

	DirectX::XMMATRIX Isekai::GetMatrix(DirectX::INT3 index)
	{
		resolve_index(index);
	...
	DirectX::INT3 Isekai::SetAllRef(DirectX::INT3 ref, DirectX::INT3 index)
	{
		SetPosRef(ref, index);
		SetRotRef(ref, index);
		SetScaRef(ref, index);
	...
	DirectX::INT3 Isekai::SetPosRef(DirectX::INT3 ref, DirectX::INT3 index)
	{
		resolve_index(index);
	...
	DirectX::INT3 Isekai::SetRotRef(DirectX::INT3 ref, DirectX::INT3 index)
	{
		resolve_index(index);
	...
	DirectX::INT3 Isekai::SetScaRef(DirectX::INT3 ref, DirectX::INT3 index)
	{
		resolve_index(index);
	...
	DirectX::INT3 Isekai::SetPosition(DirectX::XMFLOAT3 pos, Setup setup, DirectX::INT3 index)
	{
		/*
			in both cases we place vector in ref world
			however in GET the vector already exists and is the subtraction of origins
	...
	DirectX::INT3 Isekai::SetRotation(DirectX::XMFLOAT3 rot, Setup setup, DirectX::INT3 index)
	{
		resolve_index(index);
	...
	DirectX::INT3 Isekai::SetScale(DirectX::XMFLOAT3 scale, Setup setup, DirectX::INT3 index)
	{
	...
	DirectX::INT3 Isekai::SetOrigin(DirectX::XMFLOAT3 origin, DirectX::INT3 index)
	{
		resolve_index(index);
	...
	DirectX::INT3 Isekai::Translate(DirectX::XMFLOAT3 pos, Setup setup, DirectX::INT3 index)
	{
		resolve_all(index, setup);
	...
	DirectX::INT3 Isekai::Rotate(DirectX::XMFLOAT3 rot, Setup setup, DirectX::INT3 index)
	{
		resolve_index(index);
		rot -= GetRotation(index, setup); // WHY ? ? ? difference?
	...
	DirectX::INT3 Isekai::Scale(DirectX::XMFLOAT3 sca, Setup setup, DirectX::INT3 index)
	{
		resolve_index(index);
		if (setup.is_signed.x)
	...
	DirectX::INT3 Isekai::Fit(DirectX::INT3 index)
	{
...
#define ugly_verytemp_neveragain_macro_TO uvs.push_back({ 1.0f, (float)i / (float)(to->nodes.size() - 1) });
	...
	void Fibre::Append(Hierarchy<Sekai>& cone, unsigned int node_index, float from, float to, Fibre::Type type)
	{
		switch (type)
		{
	...
	void Fibre::Init(Hierarchy<Sekai>& cone, unsigned int node_index, float from, float to, Fibre::Type type)
	{
		nodes.clear();
		Append(cone, node_index, from, to, type);
	}

	void Fibre::generate_peers_rec(Hierarchy<Sekai>& cone, ring<int>& peers, int parent_count, int index)
	{
	...
	DirectX::XMFLOAT3 Isekai::get_global_position(DirectX::INT3 index)
	{
		resolve_index(index);
	...
	DirectX::XMFLOAT3 Isekai::get_global_rotation(DirectX::INT3 index)
	{
		resolve_index(index);
		XMFLOAT3 result;
	...
	DirectX::XMFLOAT3 Isekai::get_global_scale(DirectX::INT3 index)
	{
		resolve_index(index);
		XMFLOAT3 result;
	...
	DirectX::XMFLOAT3 Isekai::get_global_boundary(DirectX::INT3 rotRef, DirectX::INT3 scaRef)
	{
	...
	void Isekai::resolve_index(DirectX::INT3& index)
	{
	...
	void Isekai::resolve_reference(DirectX::INT3 index, Setup& setup)
	{
		resolve_index(index);
	...
	void Isekai::resolve_pos_reference(DirectX::INT3 index, DirectX::INT3& posRef)
	{
		resolve_index(index);
		INT3 ref = GetPosRef(index);
	...
	void Isekai::resolve_rot_reference(DirectX::INT3 index, DirectX::INT3& rotRef)
	{
		resolve_index(index);
	...
	void Isekai::resolve_sca_reference(DirectX::INT3 index, DirectX::INT3& scaRef)
	{
		resolve_index(index);
	...
	void Isekai::resolve_all(DirectX::INT3& index, Setup& setup)
	{
		// resolve index
		if (index.x < 0) index.x = GetIndex();
		if (index.y < 0) index.y = GetIndex();
